...
export const userRouter = createTRPCRouter({
  ...
  claimReward: protectedProcedure.input(z.object({
    code: z.string(),
  ...
  getClaimRewardXDR: protectedProcedure.input(z.object({
    code: z.string(),
    signWith: SignUser
  })).mutation(async ({ input, ctx }) => {
    const redeemCode = input.code;
    ...
    if (getRedeem?.redeemConsumers.find((consumer) => consumer.userId === user)) {
    ...
    const tokenNumber = stroageACC.getTokenBalance(getRedeem.assetRedeem.code, getRedeem.assetRedeem.issuer);

    if (tokenNumber < 1) {
      throw new Error("Insufficient token balance");
    }
    const UserAcc = await StellarAccount.create(user);
    const trust = UserAcc.hasTrustline(getRedeem.assetRedeem.code, getRedeem.assetRedeem.issuer);
