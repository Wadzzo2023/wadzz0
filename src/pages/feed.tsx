...
import LegacyFeedPosts from "~/components/fan/creator/legacy-feed-posts";
...
function FeedContent() {
  ...
  return (
    <div
      className={
        isLegacy ? "w-full" : "mx-auto w-full max-w-7xl px-4 pb-24 pt-5 md:px-6"
      }
    >
      {isLegacy ? (
        <>
        ...
        <>
          ...
          >
            ...
            <p className="text-sm text-gray-500">
              Check out the latest stories and posts
            </p>
          </motion.div>
            ...
            >
              <div className="overflow-x-auto scrollbar-hide [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                <TabsList className="inline-flex h-auto min-w-full justify-start gap-2 bg-transparent p-0">
                  <TabsTrigger
                    value={FeedTab.All}
                    className="shrink-0 rounded-full border border-zinc-300 bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 data-[state=active]:border-zinc-900 data-[state=active]:bg-zinc-900 data-[state=active]:text-white"
                  >
                    All Posts
                  </TabsTrigger>
                  <TabsTrigger
                    value={FeedTab.Public}
                    className="flex shrink-0 items-center gap-1 rounded-full border border-zinc-300 bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 data-[state=active]:border-zinc-900 data-[state=active]:bg-zinc-900 data-[state=active]:text-white"
                  >
                    <Globe className="h-3.5 w-3.5" /> Public
                  </TabsTrigger>
                  <TabsTrigger
                    value={FeedTab.Locked}
                    className="flex shrink-0 items-center gap-1 rounded-full border border-zinc-300 bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 data-[state=active]:border-zinc-900 data-[state=active]:bg-zinc-900 data-[state=active]:text-white"
                  >
                    <Lock className="h-3.5 w-3.5" /> Locked
                  </TabsTrigger>
                  <TabsTrigger
                    value={FeedTab.Followed}
                    className="flex shrink-0 items-center gap-1 rounded-full border border-zinc-300 bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 data-[state=active]:border-zinc-900 data-[state=active]:bg-zinc-900 data-[state=active]:text-white"
                  >
      ...
      <div
        className={cn(
          "mx-auto w-full max-w-2xl overflow-hidden border-x border-zinc-200 bg-white",
          isLegacy && "mt-10 max-w-none border-x-0 bg-transparent",
        )}
      >
        {posts.isLoading ? (
          <Loading />
          ...
          <AnimatePresence initial={false} mode="sync">
            <motion.div
              ...
              className={cn(
                "space-y-0",
                isLegacy && "flex w-full flex-col items-center gap-6",
              )}
            >
                ...
                <div key={`page-${pageIndex}`}>
                  {page.posts
                    .filter((post) => {
                      ...
                      if (activeTab === FeedTab.Public)
                        return !post.subscription;
                      if (activeTab === FeedTab.Locked)
                        return !!post.subscription;
                      if (activeTab === FeedTab.Followed) {
                        const creatorWithFollowers = post.creator as {
                          followers?: unknown;
                        };
                        return (
                          Array.isArray(creatorWithFollowers.followers) &&
                          creatorWithFollowers.followers.length > 0
                        );
                      }
                      return true;
                    })
                    .map((post, index) => {
                      ...
                      if (locked && post.subscription) {
                        ...
                        const bal = getAssetBalanceFromBalance({
                          balances: accBalances.data as Parameters<
                            typeof getAssetBalanceFromBalance
                          >[0]["balances"],
                          code: pageAssetCode,
