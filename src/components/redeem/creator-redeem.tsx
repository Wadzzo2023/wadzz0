...
const RedeeemPage = () => {
  ...
  if (creator.isLoading) {
    return (
      <Dialog>
        <DialogContent className="flex max-h-[60%] min-h-[60%] min-w-[60%] flex-col items-start overflow-y-auto">
...
const Redeem = ({ creatorId }: { creatorId: string }) => {
  ...
  const totalFees = api.fan.trx.getRequiredPlatformAsset.useQuery({
    xlm: TRUST_XLM, // this is for the trust fee and default (trx base + platform fee)
  });
          ...
          <TableBody>
            {creatorAssets.data.map((asset) => (
              <TableRow key={asset.id} className="">
                ...
                <TableCell>
                  {asset.Redeem[asset.Redeem.length - 1]?.code ??
                    "No Redeem Code"}{" "}
                </TableCell>
                <TableCell>
                  {asset.Redeem[asset.Redeem.length - 1]?.remaining ??
                    "No Redeem Code"}{" "}
                </TableCell>
                <TableCell>
                  <Button
                    disabled={(() => {
                      const lastRedeem =
                        asset.Redeem?.[asset.Redeem.length - 1];
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Set the maximum number of times this code can be redeemed for
            ...
            <DialogFooter>
              <div className="flex w-full flex-col gap-2">
                <PaymentChoose
                  costBreakdown={[
                    {
                      ...
                      amount:
                        paymentMethod === "asset"
                          ...
                          : Number(TRUST_XLM + TrxBaseFee) *
                              getValues("maxRedeems") +
                            xlmPlatformFee,
                      highlighted: true,
                      type: "cost",
                    },
                  ...
                  XLM_EQUIVALENT={
                    Number(TRUST_XLM + TrxBaseFee) * getValues("maxRedeems") +
                    xlmPlatformFee
                  }
